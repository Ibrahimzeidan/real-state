...
export function Footer() {
  return (
    <footer className="bg-secondary/90 text-accent border-t border-border/60">
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="inline-block cursor-pointer group">
              <span className="text-2xl font-serif font-bold transition-colors group-hover:text-accent">
                ibrahim's company<span className="text-accent">.</span>
              </span>
            </Link>
            <p className="mt-4 text-sm text-accent/80 leading-relaxed">
            ...
            <ul className="mt-4 space-y-3">
              {footerLinks.company.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-sm text-accent/80 transition-all duration-200 hover:text-accent hover:translate-x-1 inline-block cursor-pointer"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3">
              <li className="flex items-center gap-3 text-sm text-accent/80 group cursor-pointer hover:text-accent transition-colors">
        ...
        <div className="mt-12 border-t border-border/60 pt-8">
          <p className="text-center text-sm text-accent/70">
            &copy; {new Date().getFullYear()} Ibrahim Company. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
