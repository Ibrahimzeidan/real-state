...
import { AdminCredentialsForm } from "@/components/dashboard/admin-credentials-form";
import { ContentPageManager } from "@/components/dashboard/content-page-manager";
...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    PageContent.find()
      .select("pageName createdAt updatedAt")
      .sort({ updatedAt: -1 })
      .lean(),
  ...
  auditRows.forEach((row: any) => {
    activities.push({
      ...
      user: getInitials(row.actorId?.name || row.actorName),
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <section id="admin-access" className="scroll-mt-24">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Update the admin email or password stored in `frontend/.env`.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <AdminCredentialsForm currentEmail={authUser.email} />
          </CardContent>
        </Card>
      </section>
      ...
      <section id="content" className="scroll-mt-24 space-y-6">
        ...
        <ContentPageManager initialPages={pagesForManagement} />
                  ...
                  {auditRows.length ? (
                    auditRows.map((row: any) => (
                      <TableRow key={row._id.toString()}>
                        ...
                        <TableCell className="text-muted-foreground">
                          {row.actorId?.name || row.actorName || "System"}
                        </TableCell>
                        <TableCell className="text-muted-foreground">
                          {formatDate(row.createdAt)}
                        </TableCell>
