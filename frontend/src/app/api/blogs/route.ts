...
import { blogSchema, paginationSchema } from "@/lib/validation";
...
export async function GET(req: NextRequest) {
  try {
    ...
    return jsonError("Unable to fetch blogs.", 500);
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ data: normalizeDoc(blog) }, { status: 201 });
  } catch (error) {
